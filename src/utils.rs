...
/// Low-level library errors.
///
/// For high-level compilation/validation status output, consult [`CompilationResult`].
#[derive(Error, Debug)]
...
impl OperationOutput {
    ...
    pub fn from_operation_result(result: DxcOperationResult) -> Result<Self> {
        ...
        if result.get_status()?.is_err() {
            assert!(output.as_ref().is_empty());
            Err(HassleError::OperationError(error.to_owned()))
        } else {
            assert!(!output.as_ref().is_empty());
            Ok(OperationOutput {
